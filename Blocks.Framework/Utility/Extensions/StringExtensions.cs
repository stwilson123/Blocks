...
namespace Blocks.Framework.Utility.Extensions
{
    public static class StringExtensions
    {
        public static string CamelFriendly(this string camel)
        {
            ...
            for (var i = camel.Length - 1; i > 0; i--) if (char.IsUpper(sb[i])) sb.Insert(i, ' ');
        ...
        public static string Ellipsize(this string text, int characterCount, string ellipsis, bool wordBoundary = false)
        {
            ...
            while (characterCount > 0 && text[characterCount - 1].IsLetter()) characterCount--;

            // search previous word
            while (characterCount > 0 && text[characterCount - 1].IsSpace()) characterCount--;

            // if it was the last word, recover it, unless boundary is requested
            if (characterCount == 0 && !wordBoundary) characterCount = backup;
        ...
        public static string HtmlClassify(this string text)
        {
            ...
            for (var i = 0; i < friendlier.Length; i++)
            {
                ...
                {
                    if (previousIsNotLetter && i != 0 && cursor > 0) result[cursor++] = '-';
...
//        }
        ...
        public static string RemoveTags(this string html, bool htmlDecode = false)
        {
            ...
            for (var i = 0; i < html.Length; i++)
            {
                ...
                if (!inside) result[cursor++] = current;
            }

            var stringResult = new string(result, 0, cursor);

            if (htmlDecode) stringResult = HttpUtility.HtmlDecode(stringResult);
        ...
        {
            return Enumerable.Range(0, hex.Length).Where(x => 0 == x % 2)
        ...
        public static string ToSafeName(this string name)
        {
            ...
            while (name.Length > 0 && !IsLetter(name[0])) name = name.Substring(1);
        ...
        public static string Translate(this string subject, char[] from, char[] to)
        {
            ...
            {
                var current = subject[i];
                if (map.ContainsKey(current)) result[i] = map[current];
