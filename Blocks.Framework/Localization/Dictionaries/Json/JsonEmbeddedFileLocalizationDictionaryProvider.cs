...
namespace Blocks.Framework.Localization.Dictionaries.Json
{
    ...
    public class JsonEmbeddedFileLocalizationDictionaryProvider : LocalizationDictionaryProviderBase
    {
        ...
        /// </para>
        /// </param>
        public JsonEmbeddedFileLocalizationDictionaryProvider(string sourceName, Assembly assembly, string rootNamespace) : base(sourceName)
        {
            _assembly = assembly;
            ...
            foreach (var resourceName in resourceNames)
            {
                if (resourceName.StartsWith(_rootNamespace))
                {
                    using (var stream = _assembly.GetManifestResourceStream(resourceName))
                    {
                        ...
                        var jsonString = await Utf8Helper.ReadStringFromStreamAsync(stream);
