                ...
                try {
                    ...
                    if (descriptor.Path != null && !descriptor.Path.IsValidUrlSegment()) {
                        Logger.ErrorFormat("The module '{0}' could not be loaded because it has an invalid Path ({1}). It was ignored. The Path if specified must be a valid URL segment. The best bet is to stick with letters and numbers with no spaces.",
                                     extensionId,
                                     descriptor.Path);
                        _criticalErrorProvider.RegisterErrorMessage(T.LocalizationSource.GetString(string.Format("The extension '{0}' could not be loaded because it has an invalid Path ({1}). It was ignored. The Path if specified must be a valid URL segment. The best bet is to stick with letters and numbers with no spaces.",
                                     extensionId,
                                     descriptor.Path)));
                        continue;
                    }
                    ...
                    _criticalErrorProvider.RegisterErrorMessage(T.LocalizationSource.GetString(string.Format("The extension '{0}' manifest could not be loaded. It was ignored.", extensionId)));
                }
            }
